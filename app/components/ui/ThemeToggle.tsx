"use client";

// components/ui/ThemeToggle.tsx
// Tombol toggle dark/light mode.
// Sun = light, Moon = dark.
// Icon swap pakai CSS clip + translate — tidak ada library animasi.

import { useTheme } from "@/hooks/useTheme";

interface ThemeToggleProps {
    size?: "sm" | "md";
}

export default function ThemeToggle({ size = "md" }: ThemeToggleProps) {
    const { isDark, toggleTheme } = useTheme();

    const btnSize = size === "sm" ? "w-8 h-8" : "w-9 h-9";
    const iconSize = size === "sm" ? 14 : 16;

    return (
        <button
            onClick={toggleTheme}
            className={`${btnSize} flex items-center justify-center rounded-full text-sumi-muted hover:text-sumi hover:bg-sumi/5 transition-colors relative overflow-hidden`}
            title={isDark ? "Switch to light mode" : "Switch to dark mode"}
            aria-label={isDark ? "Aktifkan mode terang" : "Aktifkan mode gelap"}
        >
            {/* Sun icon */}
            <svg
                width={iconSize}
                height={iconSize}
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
                className="absolute transition-all duration-300"
                style={{
                    opacity: isDark ? 0 : 1,
                    transform: isDark ? "scale(0.5) rotate(90deg)" : "scale(1) rotate(0deg)",
                }}
            >
                <circle cx="12" cy="12" r="4" />
                <line x1="12" y1="2" x2="12" y2="4" />
                <line x1="12" y1="20" x2="12" y2="22" />
                <line x1="4.22" y1="4.22" x2="5.64" y2="5.64" />
                <line x1="18.36" y1="18.36" x2="19.78" y2="19.78" />
                <line x1="2" y1="12" x2="4" y2="12" />
                <line x1="20" y1="12" x2="22" y2="12" />
                <line x1="4.22" y1="19.78" x2="5.64" y2="18.36" />
                <line x1="18.36" y1="5.64" x2="19.78" y2="4.22" />
            </svg>

            {/* Moon icon */}
            <svg
                width={iconSize}
                height={iconSize}
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
                className="absolute transition-all duration-300"
                style={{
                    opacity: isDark ? 1 : 0,
                    transform: isDark ? "scale(1) rotate(0deg)" : "scale(0.5) rotate(-90deg)",
                }}
            >
                <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
            </svg>
        </button>
    );
}